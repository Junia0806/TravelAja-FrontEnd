...
import { fetchPromotions } from "../Redux/actions/promoActions";
import { setPage } from "../Redux/reducers/promoReducers";
import { Link } from "react-router-dom";

const PromoPagination = () => {
  const dispatch = useDispatch();
  const { promotions, page} = useSelector((state) => state.promo);

  useEffect(() => {
    dispatch(fetchPromotions(page));
  }, [dispatch, page]);
            ...
            <div key={index} className="relative bg-white rounded-lg shadow-lg overflow-hidden mb-8">
              <div className="absolute top-0 right-0 bg-red-700 text-white py-1 px-4 rounded-bl-lg">
                <p className="text-sm font-semibold">
                  {promotion?.seatclass?.seat_class_type} -{promotion.promotion.discount}%
                </p>
              </div>
              <div className="h-48 w-full object-cover bg-white">
                <img src={promotion?.airlines?.url_logo} className="h-48 w-full" alt="Airline Logo" />
              </div>
              <div className="p-4">
                <p className="font-bold text-center text-xl mb-1 text-black">
                  {promotion.destination_airport?.city} <i className="fa-solid fa-arrow-right"></i> {promotion.arrival_airport?.city}
                </p>
                <div className="flex justify-center space-x-10 text-gray-700 mb-1">
                  <p className="font-medium text-black">
                    <i className="fa-solid fa-plane-circle-check"></i> {promotion.airlines?.airline_name}
                  </p>
                  <p className="font-medium text-black">
                    <i className="fa-regular fa-calendar"></i> {formatDate(promotion.date)}
                  </p>
                </div>

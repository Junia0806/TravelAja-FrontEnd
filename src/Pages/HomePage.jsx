...
const Home = () => {
    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          `https://expressjs-production-6cc1.up.railway.app/api/v1/airport`
        );
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get(
          "https://expressjs-production-6cc1.up.railway.app/api/v1/seatclasses"
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        `https://expressjs-production-6cc1.up.railway.app/api/v1/search?query=${formData.departureAirport?.name}`
      );

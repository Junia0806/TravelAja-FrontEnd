...
export const proceedToPayment = (payload, token) => async (dispatch) => {
  try {
    const res = await axios.post(
      "https://expressjs-production-7255.up.railway.app/api/v1/booking/process",
      payload,
      {
        headers: {
...
export const fetchBookingHistory = () => async (dispatch, getState) => {
  try {
    ...
    const res = await axios.get(
      `https://expressjs-production-7255.up.railway.app/api/v1/booking/history`,
...
export const fetchDetailBooking =
  (bookingCode) => async (dispatch, getState) => {
    try {
      const { token } = getState().auth;
      const res = await axios.get(
        `https://expressjs-production-7255.up.railway.app/api/v1/booking/id/${bookingCode}`,
